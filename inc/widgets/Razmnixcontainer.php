...
class Razmnixcontainer extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'ContainerPhoenix', 'razmnix' );
	}
			...
			'container_grid' => [
				'replacements' => [
					...
					'custom'   => [
						...
						'preset_settings' => [
							'container_type' => 'grid',
							'presetTitle'    => esc_html__( 'Grid', 'razmnix' ),
	...
	protected function get_initial_config() {
		...
		$config['categories']    = [ 'razmnixWidgets' ];
	...
	protected function get_container_type_control_options( $is_container_grid_active ) {
		if ( $is_container_grid_active ) {
			return [
				'label'            => esc_html__( 'Container Layout', 'razmnix' ),
				'type'             => Controls_Manager::SELECT,
				'default'          => 'flex',
				'prefix_class'     => 'e-',
				'options'          => [
					'flex' => esc_html__( 'Flexbox', 'razmnix' ),
					'grid' => esc_html__( 'Grid', 'razmnix' ),
				],
				'selectors'        => [
	...
	protected function register_container_layout_controls() {
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Container', 'razmnix' ),
		...
		$this->add_control(
			'content_width',
			[
				'label'            => esc_html__( 'Content Width', 'razmnix' ),
				'type'             => Controls_Manager::SELECT,
				'default'          => 'boxed',
				'options'          => [
					'boxed' => esc_html__( 'Boxed', 'razmnix' ),
					'full'  => esc_html__( 'Full Width', 'razmnix' ),
				],
		...
		$width_control_settings = [
			'label'               => esc_html__( 'Width', 'razmnix' ),
		...
		$this->add_responsive_control(
			'min_height',
			[
				'label'       => esc_html__( 'Min Height', 'razmnix' ),
				'type'        => Controls_Manager::SLIDER,
				...
				'description' => sprintf(
					esc_html__( 'To achieve full height Container use %s.', 'razmnix' ),
					'100vh'
				),
				'selectors'   => [
	...
	protected function register_items_layout_controls() {
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Additional Options', 'razmnix' ),
				'tab'   => Controls_Manager::TAB_LAYOUT,
			]
		);
		...
		$this->add_control(
			'overflow',
			[
				'label'     => esc_html__( 'Overflow', 'razmnix' ),
				'type'      => Controls_Manager::SELECT,
				'default'   => '',
				'options'   => [
					''       => esc_html__( 'Default', 'razmnix' ),
					'hidden' => esc_html__( 'Hidden', 'razmnix' ),
					'auto'   => esc_html__( 'Auto', 'razmnix' ),
				],
				'selectors' => [
		...
		$this->add_control(
			'html_tag',
			[
				'label'   => esc_html__( 'HTML Tag', 'razmnix' ),
				'type'    => Controls_Manager::SELECT,
				'options' => $options,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'alert_type' => 'warning',
				'content'    => esc_html__( 'Don’t add links to elements nested in this container - it will break the layout.', 'razmnix' ),
				'condition'  => [
					'html_tag' => 'a',
				],
			]
	...
	protected function register_background_controls() {
		$this->start_controls_section(
			'section_background',
			[
				'label' => esc_html__( 'Background', 'razmnix' ),
		...
		$this->start_controls_tab(
			...
			[
				'label' => esc_html__( 'Normal', 'razmnix' ),
			]
		);
		...
		$this->start_controls_tab(
			...
			[
				'label' => esc_html__( 'Hover', 'razmnix' ),
			]
		);
		...
		$this->add_control(
			...
			[
				'label'       => esc_html__( 'Transition Duration', 'razmnix' ) . ' (s)',
				'type'        => Controls_Manager::SLIDER,
				'default'     => [
					'size' => 0.3,
				],
	...
	protected function register_background_overlay_controls() {
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Background Overlay', 'razmnix' ),
				'tab'   => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				'label'     => esc_html__( 'Opacity', 'razmnix' ),
				'type'      => Controls_Manager::SLIDER,
				'default'   => [
					'size' => .5,
				],
		...
		$this->add_control(
			'overlay_blend_mode',
			[
				'label'      => esc_html__( 'Blend Mode', 'razmnix' ),
				'type'       => Controls_Manager::SELECT,
				'options'    => [
					''            => esc_html__( 'Normal', 'razmnix' ),
					'multiply'    => esc_html__( 'Multiply', 'razmnix' ),
					'screen'      => esc_html__( 'Screen', 'razmnix' ),
					'overlay'     => esc_html__( 'Overlay', 'razmnix' ),
					'darken'      => esc_html__( 'Darken', 'razmnix' ),
					'lighten'     => esc_html__( 'Lighten', 'razmnix' ),
					'color-dodge' => esc_html__( 'Color Dodge', 'razmnix' ),
					'saturation'  => esc_html__( 'Saturation', 'razmnix' ),
					'color'       => esc_html__( 'Color', 'razmnix' ),
					'luminosity'  => esc_html__( 'Luminosity', 'razmnix' ),
				],
				'selectors'  => [
	...
	protected function register_border_controls() {
		$this->start_controls_section(
			'section_border',
			[
				'label' => esc_html__( 'Border', 'razmnix' ),
				'tab'   => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_responsive_control(
			'border_radius',
			[
				'label'      => esc_html__( 'Border Radius', 'razmnix' ),
	...
	protected function register_shape_dividers_controls() {
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Shape Divider', 'razmnix' ),
				'tab'   => Controls_Manager::TAB_STYLE,
			]
		);
		...
		foreach (
			[
				'top'    => esc_html__( 'Top', 'razmnix' ),
				'bottom' => esc_html__( 'Bottom', 'razmnix' ),
			] as $side => $side_label
		) {
			...
			$this->add_control(
				...
				[
					'label'     => esc_html__( 'Color', 'razmnix' ),
			...
			$this->add_responsive_control(
				...
				[
					'label'      => esc_html__( 'Height', 'razmnix' ),
					'type'       => Controls_Manager::SLIDER,
					'size_units' => [ 'px', 'em', 'rem', 'custom' ],
					'range'      => [
						'px'  => [
			...
			$this->add_control(
				...
				[
					'label'              => esc_html__( 'Invert', 'razmnix' ),
					'type'               => Controls_Manager::SWITCHER,
					'frontend_available' => true,
					'condition'          => [
			...
			$this->add_control(
				...
				[
					'label'     => esc_html__( 'Bring to Front', 'razmnix' ),
					'type'      => Controls_Manager::SWITCHER,
					'selectors' => [
	...
	protected function register_advanced_controls() {
		$this->start_controls_section(
			'section_layout',
			[
				'label' => esc_html__( 'Layout', 'razmnix' ),
		...
		$this->add_responsive_control(
			'margin',
			[
				'label'      => esc_html__( 'Margin', 'razmnix' ),
				'type'       => Controls_Manager::DIMENSIONS,
				'size_units' => [ 'px', '%', 'em', 'rem', 'vw', 'custom' ],
				'selectors'  => [
		...
		$this->add_responsive_control(
			'padding',
			[
				'label'      => esc_html__( 'Padding', 'razmnix' ),
				'type'       => Controls_Manager::DIMENSIONS,
				'size_units' => [ 'px', '%', 'em', 'rem', 'vw', 'custom' ],
				'selectors'  => [
		...
		$this->add_control(
			...
			[
				'type'        => Controls_Manager::ALERT,
				'alert_type'  => 'warning',
				'heading'     => esc_html__( 'Please note!', 'razmnix' ),
				'content'     => esc_html__( 'Custom positioning is not considered best practice for responsive web design and should not be used too frequently.', 'razmnix' ),
				'render_type' => 'ui',
				'condition'   => [
					'position!' => '',
				],
		...
		$this->add_control(
			'position',
			[
				'label'              => esc_html__( 'Position', 'razmnix' ),
				'type'               => Controls_Manager::SELECT,
				'default'            => '',
				'options'            => [
					''         => esc_html__( 'Default', 'razmnix' ),
					'absolute' => esc_html__( 'Absolute', 'razmnix' ),
		...
		$left  = esc_html__( 'Left', 'razmnix' );
		$right = esc_html__( 'Right', 'razmnix' );
		...
		$this->add_control(
			...
			[
				'label'       => esc_html__( 'Horizontal Orientation', 'razmnix' ),
		...
		$this->add_responsive_control(
			'_offset_x',
			[
				'label'      => esc_html__( 'Offset', 'razmnix' ),
				'type'       => Controls_Manager::SLIDER,
				'range'      => [
					'px' => [
						'min' => - 1000,
		...
		$this->add_control(
			...
			[
				'label'       => esc_html__( 'Vertical Orientation', 'razmnix' ),
				'type'        => Controls_Manager::CHOOSE,
				'toggle'      => false,
				'default'     => 'start',
				'options'     => [
					'start' => [
						'title' => esc_html__( 'Top', 'razmnix' ),
						'icon'  => 'eicon-v-align-top',
					],
					'end'   => [
						'title' => esc_html__( 'Bottom', 'razmnix' ),
		...
		$this->add_responsive_control(
			'z_index',
			[
				'label'     => esc_html__( 'Z-Index', 'razmnix' ),
		...
		$this->add_control(
			'_element_id',
			[
				'label'          => esc_html__( 'CSS ID', 'razmnix' ),
				...
				'title'          => esc_html__( 'Add your custom id WITHOUT the Pound key. e.g: my-id', 'razmnix' ),
		...
		$this->add_control(
			'css_classes',
			[
				'label'        => esc_html__( 'CSS Classes', 'razmnix' ),
				'type'         => Controls_Manager::TEXT,
				'default'      => '',
				'ai'           => [
					'active' => false,
				...
				'prefix_class' => '',
				'title'        => esc_html__( 'Add your custom class WITHOUT the dot. e.g: my-class', 'razmnix' ),
				'classes'      => 'elementor-control-direction-ltr',
			]
		);
	...
	protected function register_motion_effects_controls() {
		$this->start_controls_section(
			'section_effects',
			[
				'label' => esc_html__( 'Motion Effects', 'razmnix' ),
				'tab'   => Controls_Manager::TAB_ADVANCED,
			]
		);
		...
		$this->add_responsive_control(
			'animation',
			[
				'label'              => esc_html__( 'Entrance Animation', 'razmnix' ),
		...
		$this->add_control(
			'animation_duration',
			[
				'label'        => esc_html__( 'Animation Duration', 'razmnix' ),
				'type'         => Controls_Manager::SELECT,
				'default'      => '',
				'options'      => [
					'slow' => esc_html__( 'Slow', 'razmnix' ),
					''     => esc_html__( 'Normal', 'razmnix' ),
					'fast' => esc_html__( 'Fast', 'razmnix' ),
				],
		...
		$this->add_control(
			'animation_delay',
			[
				'label'              => esc_html__( 'Animation Delay', 'razmnix' ) . ' (ms)',
				'type'               => Controls_Manager::NUMBER,
				'default'            => '',
				'min'                => 0,
				'step'               => 100,
	...
	protected function register_responsive_controls() {
		$this->start_controls_section(
			'_section_responsive',
			[
				'label' => esc_html__( 'Responsive', 'razmnix' ),
				'tab'   => Controls_Manager::TAB_ADVANCED,
			]
		);
		...
		$this->add_control(
			'heading_visibility',
			[
				'label' => esc_html__( 'Visibility', 'razmnix' ),
		...
		$this->add_control(
			...
			[
				...
				/* translators: 1: Link open tag, 2: Link close tag. */
					esc_html__( 'Responsive visibility will take effect only on %1$s preview mode %2$s or live page, and not while editing in Elementor.', 'razmnix' ),
	...
	protected function hook_sticky_notice_into_transform_section() {
		add_action( 'elementor/element/container/_section_transform/after_section_start', function ( \Elementor\Includes\Elements\Container $container ) {
			...
			$container->add_control(
				...
				[
					'type'       => Controls_Manager::ALERT,
					'alert_type' => 'warning',
					'content'    => esc_html__( 'Note: Avoid applying transform properties on sticky containers. Doing so might cause unexpected results.', 'razmnix' ),
				]
			);
		} );
	}
